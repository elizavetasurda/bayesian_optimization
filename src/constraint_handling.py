...
class ConstraintHandler:
    ...
    def compute_acquisition(self, mean: np.ndarray, var: np.ndarray, 
                           constraints_mean: np.ndarray, constraints_var: np.ndarray,
...
class LagrangeHandler(ConstraintHandler):
    ...
    def __init__(self, problem, lambda_init: float = 1.0):
        super().__init__(problem)
        self.lambdas = np.ones(problem.constraints(np.zeros(problem.dim)).shape[0]) * lambda_init
...
class BarrierHandler(ConstraintHandler):
    ...
    def compute_acquisition(self, mean: np.ndarray, var: np.ndarray,
        ...
        for j in range(constraints_mean.shape[1]):
            ...
            barrier_penalty += -self.mu * np.log(-g_mean + self.barrier_tol) * (1 - p_violation)
...
def get_handler(name: str, problem) -> ConstraintHandler:
    ...
    if name not in handlers:
        raise ValueError(f"Unknown handler: {name}. Choose from {list(handlers.keys())}")
