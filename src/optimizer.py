...
class BayesianOptimization:
    ...
    def _acquisition_function(self, X_candidates: np.ndarray) -> np.ndarray:
        ...
        acq = self.handler.compute_acquisition(mean_f, var_f, mean_g, var_g, f_best)
    ...
    def optimize(self, verbose: bool = True) -> OptimizationResult:
        ...
        for iteration in range(self.n_iterations):
            ...
            # Update best feasible
            if (
                self.problem.is_feasible(x_next)
                and f_next < self.best_feasible_objective
            ):
        ...
        if verbose:
            ...
            if self.best_feasible_point is not None:
                print(
                    f"[BO] Best feasible objective: {self.best_feasible_objective:.6f}"
                )
            else:
